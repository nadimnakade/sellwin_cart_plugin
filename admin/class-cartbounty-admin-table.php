	...
	*/
	...
	function get_columns(){
		return $columns = array(
			...
			'follow_up'		=>	esc_html__('Follow-up', 'woo-save-abandoned-carts')
		);
	}
	...
	function column_follow_up( $item ) {
		$admin = $this->admin();
		$current_status = isset( $item['contacted_status'] ) ? $item['contacted_status'] : 'pending';
		$whatsapp_nonce = wp_create_nonce( 'cartbounty_whatsapp_nonce' );
		$status_nonce = wp_create_nonce( 'cartbounty_status_nonce' );

		$output = '<select class="cartbounty-status-select" data-cart-id="' . esc_attr( $item['id'] ) . '" data-nonce="' . esc_attr( $status_nonce ) . '">';
		$output .= '<option value="pending"' . selected( $current_status, 'pending', false ) . '>' . esc_html__( 'Pending', 'woo-save-abandoned-carts' ) . '</option>';
		$output .= '<option value="contacted"' . selected( $current_status, 'contacted', false ) . '>' . esc_html__( 'Contacted', 'woo-save-abandoned-carts' ) . '</option>';
		$output .= '<option value="not_contacted"' . selected( $current_status, 'not_contacted', false ) . '>' . esc_html__( 'Not Contacted', 'woo-save-abandoned-carts' ) . '</option>';
		$output .= '</select>';

		if ( ! empty( $item['phone'] ) ) {
			$phone = preg_replace( '/[^0-9]/', '', $item['phone'] );
			$output .= ' <a href="https://wa.me/' . esc_attr( $phone ) . '" target="_blank" class="button cartbounty-whatsapp-btn" data-cart-id="' . esc_attr( $item['id'] ) . '" data-nonce="' . esc_attr( $whatsapp_nonce ) . '">' . esc_html__( 'WhatsApp', 'woo-save-abandoned-carts' ) . '</a>';
		}

		$download_url = '?page=' . esc_attr( $_REQUEST['page'] ) . '&action=download_pdf&id=' . esc_attr( $item['id'] );
		$output .= ' <a href="' . esc_url( $download_url ) . '" class="button">' . esc_html__( 'PDF', 'woo-save-abandoned-carts' ) . '</a>';

		if ( ! empty( $item['contacted_time'] ) && $item['contacted_time'] !== '0000-00-00 00:00:00' ) {
			$output .= '<br/><small>' . esc_html( $item['contacted_time'] ) . '</small>';
		}
