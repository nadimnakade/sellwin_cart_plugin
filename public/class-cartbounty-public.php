...
class CartBounty_Public{
	...
	function get_user_data(){
		...
		if( is_user_logged_in() ){ //If user has signed in, seed cart identity from the WordPress/WooCommerce profile
			...
			if( $billing_first_name ){
				$user_data['name'] = sanitize_text_field( $billing_first_name );
			
			}else{
				$user_data['name'] = sanitize_text_field( $current_user->user_firstname ? $current_user->user_firstname : $current_user->display_name );
			}

			if( $billing_last_name ){
				$user_data['surname'] = sanitize_text_field( $billing_last_name );
			...
			if( $billing_email ){
				$user_data['email'] = sanitize_email( $billing_email );
			...
			if( $billing_phone ){
				$user_data['phone'] = sanitize_text_field( $billing_phone );
			}elseif( !empty( $_COOKIE['sellwin_mobile'] ) ){
				$user_data['phone'] = sanitize_text_field( $_COOKIE['sellwin_mobile'] );
			}elseif( preg_match( '/^[+0-9\s().-]{7,30}$/', $current_user->user_login ) ){
				$user_data['phone'] = sanitize_text_field( $current_user->user_login );
			}

			if( $billing_country ){
				$location['country'] = sanitize_text_field( $billing_country );
			}
			...
			if( $billing_postcode ){
				$location['postcode'] = sanitize_text_field( $billing_postcode );
			}
